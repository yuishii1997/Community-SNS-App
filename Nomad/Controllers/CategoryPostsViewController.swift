...
//  Nomad-App
//
//  Created by yuishii on 2021/02/05.
...
class CategoryPostsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    @IBAction func menu(_ sender: Any) {
        let myAlert = UIAlertController(title: "投稿を全て削除しますか？", message: nil, preferredStyle: UIAlertController.Style.alert)
        // アクションを生成.
        let myAction_1 = UIAlertAction(title: "全て削除する", style: UIAlertAction.Style.default, handler: {
            (action: UIAlertAction!) in
            
            let alert: UIAlertController = UIAlertController(title: "本当に全ての投稿を削除してよろしいでしょうか？", message: nil, preferredStyle:  UIAlertController.Style.alert)
                    ...
                    self.listener1 = postsRef.addSnapshotListener() { [weak self] querySnapshot, error in
                        if let error = error {
                            ...
                            self?.postArray = querySnapshot!.documents.compactMap { document in
                                ...
                                if postData.category == self?.category && postData.isReported == false{
                                    return postData
                                } else {
...
extension CategoryPostsViewController: GADUnifiedNativeAdLoaderDelegate {
