...
class HomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    var listener_follow: ListenerRegistration!
    ...
    override func viewDidLoad() {
        ...
        let nib3 = UINib(nibName: "JustRegisteredTableViewCell", bundle: nil)
        tableView.register(nib3, forCellReuseIdentifier: "JustRegistered")
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(success), name: .success, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(success2), name: .success2, object: nil)
    ...
    @objc func success() {
        ...
        let banner = NotificationBanner(title: "投稿しました", leftView: nil, rightView: nil, style: .info, colors: nil)
    ...
    @objc func success2() {
        ...
        self.refresh(sender: nil)
        let banner = NotificationBanner(title: "コミュニティを作成しました", leftView: nil, rightView: nil, style: .info, colors: nil)
        banner.autoDismiss = false
        banner.dismissOnTap = true
        banner.show(queuePosition: .front, bannerPosition: .top, queue: .default, on: self.navigationController!)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let currentUser = Auth.auth().currentUser {
            ...
            // ログイン済み
            if listener == nil {
                fetchFollowUsers { [weak self] result in
                    switch result {
                    case .success:
                        // listener未登録なら、登録してスナップショットを受信する
                        //whereFieldとorderが"date"という同じフィールドを使用しているので、併用可
                            ...
                            self?.listener1 = postsRef.addSnapshotListener() { [weak self] querySnapshot, error in
                                if let error = error {
                                    ...
                                    self?.postArray = querySnapshot!.documents.compactMap { document in
                                        ...
                                        print("followUserArray:\(self!.followUserArray)")
                                        //自分のuidはfollowUserArrayに含まれていなくてもpostDataを返す
                                        if postData.uid == currentUser.uid && postData.isReported == false || self?.followUserArray.contains(postData.uid) == true && postData.bePrivate == "off" && postData.isReported == false && self?.blockUserArray.contains(postData.uid) == false{
                                            return postData
                                        } else {
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        if ( self.listener_follow != nil ){
            listener_follow.remove()
            listener_follow = nil
        }
    ...
    @objc func refresh(sender: UIRefreshControl?) {
        ...
        fetchBlockUsers()
        self.fetchFollowUsers { [weak self] result in
            switch result {
            case .failure(let error):
                ...
                self?.listener1 = postsRef.addSnapshotListener() { [weak self] querySnapshot, error in
                    if let error = error {
                        ...
                        self?.postArray = querySnapshot!.documents.compactMap { document in
                            ...
                            print("DEBUG_PRINT: document取得 \(document.documentID)")
                            let postData = PostData(document: document)
                            if postData.uid == uid && postData.isReported == false || self?.followUserArray.contains(postData.uid) == true && postData.bePrivate == "off" && postData.isReported == false && self?.blockUserArray.contains(postData.uid) == false {
                                return postData
                            } else {
                                print("表示しない投稿 uid=", postData.uid)
                                return nil
    ...
    func fetchFollowUsers(completion: @escaping (Result<Any?, Error>) -> Void) {
        if let myid = Auth.auth().currentUser?.uid, listener_follow == nil {
            let postsRef_follow = Firestore.firestore().collection("followUsers").document(myid)
            postsRef_follow.getDocument { [weak self] document, error in
                if let error = error {
                    ...
                    if postDic != nil {
                        if let followUsers = postDic!["users"] as? [String] {
                            self?.followUserArray = followUsers
            ...
            print("self.followUserArray.description:\(self.followUserArray.description)")
        } else {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if self.posts == "notExist" {
            ...
            //本来は100件ずつ下にスクロールしたら表示させていきたい
            if postArray.count >= 100 {
                let segment = 100 / Constants.adInterval
                if segment < Constants.numberOfAds {
                    return 100 + (segment < nativeAds.count ? segment : nativeAds.count)
                } else if Constants.numberOfAds < segment {
                    return 100 + nativeAds.count
                } else {
                    return 100
                }
            }else{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if self.posts == "notExist" {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: "JustRegistered", for: indexPath) as! JustRegisteredTableViewCell
            //セル押下時のハイライト(色が濃くなる)を無効
            cell.selectionStyle = .none
            self.tableView.separatorStyle = .none
            cell.button1.addTarget(self, action:#selector(userListButton(_:forEvent:)), for: .touchUpInside)
            return cell
        }else{
    ...
    @objc func userListButton(_ sender: UIButton, forEvent event: UIEvent) {
        
        let storyboard = UIStoryboard(name: "Main", bundle: Bundle.main)
        let userListViewController = storyboard.instantiateViewController(identifier: "UserList") as! UserListViewController
