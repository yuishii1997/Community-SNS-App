...
class ViewedViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
                            ...
                            self?.listener1 = postsRef.addSnapshotListener() { [weak self] querySnapshot, error in
                                if let error = error {
                                    ...
                                    self?.postArray = querySnapshot!.documents.compactMap { document in
                                        ...
                                        if postData.uid == currentUser?.uid && postData.isReported == false && postData.viewed == true || postData.viewed == true && postData.bePrivate == "off" && postData.isReported == false && self?.blockUserArray.contains(postData.uid) == false {
                                            return postData
                                        } else {
    ...
    @IBAction func menu(_ sender: Any) {
        let myAlert = UIAlertController(title: "閲覧履歴を全て削除しますか？", message: nil, preferredStyle: UIAlertController.Style.actionSheet)
        // アクションを生成.
        let myAction_1 = UIAlertAction(title: "全て削除する", style: UIAlertAction.Style.destructive, handler: {
            (action: UIAlertAction!) in
            
            let alert: UIAlertController = UIAlertController(title: "よろしいですか？", message: "閲覧履歴がすべて削除されます。", preferredStyle:  UIAlertController.Style.alert)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if self.posts == "notExist" {
            ...
            cell.label1.text = "まだ閲覧をしていません"
            cell.label2.text = "閲覧するとここに表示されます。"
...
extension ViewedViewController: GADUnifiedNativeAdLoaderDelegate {
