...
class Page2ComViewController: UITableViewController,SegementSlideContentScrollViewDelegate {
                ...
                self.fetchBlockUsers { [weak self] result in
                    ...
                    case .success:
                        if ( self?.listener1 == nil ) {
                            ...
                            let postsRef = Firestore.firestore().collection("posts").whereField("deleted", isEqualTo: "no").order(by: "date", descending: true)
                            self?.listener1 = postsRef.addSnapshotListener() { (querySnapshot, error) in
                                if let error = error {
                                    print("DEBUG_PRINT: snapshotの取得が失敗しました。 \(error)")
                                    return
                                }else{
...
extension Page2ComViewController: GADUnifiedNativeAdLoaderDelegate {
