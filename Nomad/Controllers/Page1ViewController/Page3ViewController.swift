...
class Page3ViewController: UITableViewController,SegementSlideContentScrollViewDelegate {
        ...
        if Auth.auth().currentUser != nil {
            self.fetchModerateUsers { [weak self] result in
                ...
                case .success:
                    ...
                    self?.listener3 = postsRef.addSnapshotListener() { [weak self] querySnapshot, error in
                        if let error = error {
    ...
    private func fetchUser2() {
        ...
        listener2 = userRef2.addSnapshotListener() { (documentSnapshot, error) in
            guard let document = documentSnapshot else {
                print("Error fetching document: \(error!)")
                return
            }
    ...
    // モデレートしているコミュニティの情報を取得
    func fetchModerateUsers(completion: @escaping (Result<Any?, Error>) -> Void) {
        if listener_moderate == nil {
            let ref_moderate = Firestore.firestore().collection("moderateUsers").document(userID)
    ...
    override func numberOfSections(in tableView: UITableView) -> Int {
        ...
        if self.userArray.count == 0 || blockUserArray.contains(self.userID)  || self.userdata?.bePrivate == "on" && self.userID != myid && self.userdata?.isFollowed == false {
            return 1
        }else{
            return 3
        }
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let myid = Auth.auth().currentUser?.uid
        if userArray.count == 0 || blockUserArray.contains(self.userID) || self.userdata?.bePrivate == "on" && self.userID != myid && self.userdata?.isFollowed == false {
            print("遷移なし")
        }else{
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if blockUserArray.contains(self.userID){
            ...
            cell.label.text = "このユーザーをブロックしています。ブロックを解除するとコンテンツが表示されます。"
            return cell
            ...
            if self.userArray.count == 0 {
                let cell = tableView.dequeueReusableCell(withIdentifier: "NotP2Posts", for: indexPath) as! NotP2PostsTableViewCell
                cell.label.text = "このユーザーはコミュニティを作成していません。"
                return cell
            }else{
                if indexPath.section == 0 {
