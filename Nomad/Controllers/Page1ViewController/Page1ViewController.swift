...
class Page1ViewController: UITableViewController,SegementSlideContentScrollViewDelegate {
    ...
    func fetchBlockUsers(completion: @escaping (Result<Any?, Error>) -> Void) {
        if let myid = Auth.auth().currentUser?.uid, listener_block == nil {
            ...
            postsRef_block.addSnapshotListener() { (documentSnapshot, error) in
                if let error = error {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if self.userdata?.bePrivate == "on" && self.userID != myid && self.userdata?.isFollowed == false {
                ...
                if userdata?.caption != nil {
                    ...
                    //addTargetの前にremoveTargetする必要がある
                    if userdata?.isBlocked == true {
                        cell.followButton.removeTarget(self, action: #selector(follow2(_:forEvent:)), for: .touchUpInside)
                        cell.followButton.removeTarget(self, action: #selector(follow(_:forEvent:)), for: .touchUpInside)
                        cell.followButton.addTarget(self, action: #selector(unBlockButton(_:forEvent:)), for: .touchUpInside)
                    }else{
                        if userdata?.isModerated == true {
                            ...
                            cell.followButton.removeTarget(self, action: #selector(unBlockButton(_:forEvent:)), for: .touchUpInside)
                            cell.followButton.removeTarget(self, action: #selector(follow(_:forEvent:)), for: .touchUpInside)
                            cell.followButton.addTarget(self, action: #selector(follow2(_:forEvent:)), for: .touchUpInside)
                        }
                    }
    ...
    @objc func unBlockButton(_ sender: UIButton, forEvent event: UIEvent) {
        
        // styleをActionSheetに設定
        let alertSheet = UIAlertController(title: "このユーザーのブロックを解除しますか？", message: nil, preferredStyle: UIAlertController.Style.actionSheet)
        
        // 自分の選択肢を生成
        let action1 = UIAlertAction(title: "ブロックを解除", style: UIAlertAction.Style.destructive, handler: {
            ...
            if Auth.auth().currentUser != nil {
                ...
                updateValue1 = FieldValue.arrayRemove([["uid": myid, "time": self.userdata!.blockedTime]])
                updateValue2 = FieldValue.arrayRemove([self.userdata!.uid])
                updateValue3 = FieldValue.arrayRemove([myid])
                updateValue4 = FieldValue.arrayRemove([["uid": self.userdata2!.uid, "time": self.userdata2!.blockingTime]])
                
                let userRef = Firestore.firestore().collection("users").document(self.userdata!.uid)
                userRef.updateData(["blocked": updateValue1])
                ...
                let blockedRef = Firestore.firestore().collection("blockedUsers").document(self.userdata!.uid)
    ...
    @objc func follow2(_ sender: UIButton, forEvent event: UIEvent) {
        
        print("follow2")
        let dialog = UIAlertController(title: "非公開アカウント", message: "このアカウントは非公開のため、フォローすることができません。", preferredStyle: .alert)
        //ボタンのタイトル
...
extension Page1ViewController: GADUnifiedNativeAdLoaderDelegate {
