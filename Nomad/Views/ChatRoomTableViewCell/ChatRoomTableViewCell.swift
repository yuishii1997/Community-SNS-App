...
//
// ChatRoomViewControllerのコメントのセル
...
class ChatRoomTableViewCell: UITableViewCell {
    ...
    func setMessageData(_ messageData: Message) {
        ...
        if Auth.auth().currentUser != nil {
            ...
            messageData.message.enumerateLines { (line, stop) -> () in
                ...
                if self.lineIndex == 1 {
                    ...
                    if line.contains(">>") {
                        // UILabelの文字列から、attributedStringを作成します.
                        let attrText = NSMutableAttributedString(string: self.messageLabel.text!)
                        // rangeで該当箇所を指定します（ここでは「いいえ」が対象）.
                        attrText.addAttribute(.foregroundColor,
                                              value: UIColor.blue, range: NSMakeRange(0, line.count))
...
extension Date {
    func readableDate(start: Date, end: Date) -> String  {
        
        let secondsAgo = Int(end.timeIntervalSince(start))
        ...
        //確かめる用
        //一時間以上の投稿に"・7日以上"とつき、YY/MM/DD　で表示される
        //        let value: Int
        //        let unit: String
        ...
        //else if secondsAgo < month以降は全て日付にする
